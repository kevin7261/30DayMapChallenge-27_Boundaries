<script>
   ...
   *    ✓ 根據 level 屬性繪製優雅的藍綠到紅橙漸變網格
   *    ✓ 使用 Viridis 配色映射風險等級（最上層）
   *
   * 3. 視覺元素：
   *    ✓ 縣市區域：白色填充，深藍灰邊框（底層）
   *    ✓ 登革熱網格：Viridis 漸變填充，無邊框（最上層）
   *    ✓ 淺灰藍色地圖背景
   *
   * 4. 交互功能：
   *    ✓ 滾輪縮放控制
        ...
        try {
          ...
          g.selectAll('.dengue-grid')
            ...
            .attr('fill', (d) => {
              ...
              if (level === 0) {
                return '#f8f9fa'; // level 0：淺灰白色
              }
              return colorScale(level);
            })
            .attr('fill-opacity', (d) => {
              const level = d.properties.level;
              if (level === 0) return 0.4;
              // 根據 level 調整透明度，使顏色更鮮豔
              return Math.min(0.85, 0.6 + (level / maxLevel) * 0.25);
            })
      ...
      const createMap = () => {
        ...
        try {
          ...
          svg = d3
            ...
            .attr('width', width)
            .attr('height', height)
            .style('background', '#f5f6fa'); // 淺灰藍色背景
